...
<template>
  <a-space direction="vertical" size="large" style="width: 100%; max-width: 520px">
    <a-card title="Login" size="small">
      <a-form name="login" :model="loginModel" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
    ...
    <a-space>
      <a-button type="primary" @click="syncNickname"> Sync username → nickname </a-button>
      <a-button @click="resetAll"> Reset all </a-button>
    </a-space>
  </a-space>
</template>
```
